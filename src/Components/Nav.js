...
class Nav extends Component {
  render() {
    if (!this.props.user && !this.props.currentPlayer)
      return <Menu fixed="top" inverted color="teal" />
    if (this.props.user) {
      return (
        <Menu fixed="top" inverted color="teal" size="tiny">
          <Menu.Item
            ...
            name="Kata Cards"
            position="left"
            onClick={() => this.props.redirectToHome(this.props.history)}
          />
          ...
          <Dropdown item icon="bars" position="right" pointing="top right">
            <Dropdown.Menu>
              <Dropdown.Item
                onClick={() =>
                  this.props.redirectToBuyCards(this.props.history)
                }
              >
                Buy Cards
              </Dropdown.Item>
              <Dropdown.Item
                onClick={() => this.props.redirectToProfile(this.props.history)}
              >
                Profile
              </Dropdown.Item>
              <Dropdown.Item
                onClick={() => this.props.redirectToCredits(this.props.history)}
              >
                Artwork Credits
              </Dropdown.Item>
              <Dropdown.Item
                onClick={() =>
                  this.props.redirectToMusicCredits(this.props.history)
                }
              >
                Music Credits
              </Dropdown.Item>
              <Dropdown.Item
                onClick={() =>
                  this.props.setOffline(this.props.user.uid, this.props.history)
                }
              >
                Logout
              </Dropdown.Item>
...
const addDispatcher = (connector, ref, user) => ({
  ...
  redirectToMusicCredits(history) {
    history.push('/music-credits')
  }
})
